...
def main(args):
    ...
    for action, base_action in zip(actions, processed_base_actions):
        ...
        ts = env.step(action) # without TRACER
        #ts = env.step(action, base_action, get_tracer_vel=True) #with TRACER
        #obs_wheels.append(ts.observation['base_vel']) #with TRACER
        #obs_tracer.append(ts.observation['tracer_vel']) #with TARCER
